...
float AFEC_Init( void )
{
    ...
    AFEC0_REGS->AFEC_CSELR = (uint32_t)AFEC_CH5; AFEC0_REGS->AFEC_COCR = 512U;   /* PB2 = PRES_SENSE5 */
    ...
    AFEC1_REGS->AFEC_CSELR = (uint32_t)AFEC_CH6; AFEC1_REGS->AFEC_COCR = 512U;   /* PC31 = PRES_SENSE1 */

    /* 아날로그 입력 핀 전환 (읽는 채널 전부 명시: 입력+풀없음) */
    PIOC_REGS->PIO_ODR  = (1UL<<29)|(1UL<<30)|(1UL<<15)|(1UL<<12)|(1UL<<31);   /* PC29/30 SEN, PC15/12 P28V, PC31 PRES_SENSE1 */
    PIOC_REGS->PIO_PUDR = (1UL<<29)|(1UL<<30)|(1UL<<15)|(1UL<<12)|(1UL<<31);
    PIOD_REGS->PIO_ODR  = (1UL << 30);                                /* PD30 PRES_SENSE2 */
